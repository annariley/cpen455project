...
def get_label(model, model_input, device):
    ...
    max_lob_prob = 0
    for i in [0, 1, 2, 3]:
        label_tensor = torch.full((model_input.shape[0],), i, dtype=torch.long)
        answer = model(model_input, label_tensor, device)
        print(model_input.shape)
        print(answer.shape)
        log_prob = discretized_mix_logistic_loss(model_input, answer)
        #print(log_prob)
        #print(log_prob > max_lob_prob)
        if log_prob > max_lob_prob:
            max_lob_prob = log_prob
            max_label = i
    return max_label
# End of your code
...
if __name__ == '__main__':
    ...
    model = PixelCNN(nr_resnet=1, nr_filters=160, 
