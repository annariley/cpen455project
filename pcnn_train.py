...
label_to_index = {'Class0': 0, 'Class1': 1, 'Class2': 2, 'Class3': 3}
def train_or_test(model, data_loader, optimizer, loss_op, device, args, epoch, mode = 'training'):
    ...
    for batch_idx, item in enumerate(tqdm(data_loader)):
        ...
        label_indices = [label_to_index[lbl] for lbl in label]
        label_tensor = torch.tensor(label_indices, dtype=torch.long).to(device)
        model_output = model(model_input, label_tensor)
    ...
    for epoch in tqdm(range(args.max_epochs)):
        ...
        if epoch % args.sampling_interval == 0:
            ...
            for i in [0, 1, 2, 3]:
                ...
                save_images(sample_t, args.sample_dir, i)
                sample_result = wandb.Image(sample_t, caption="epoch {} class {}".format(epoch, i))
